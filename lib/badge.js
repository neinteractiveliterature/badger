...
function prepData(positions, data){
    ...
    for (const field of positions){
        ...
        if (field.substitutions && userData.badgeData[badgeField]){
            const weekday = days[new Date().getDay()];
            userData.badgeData[badgeField] = userData.badgeData[badgeField].replace(/%weekday%/g, weekday);
            userData.badgeData[badgeField] = userData.badgeData[badgeField].replace(/%weekday3%/g, `${weekday} ${weekday} ${weekday}`);
            userData.badgeData[badgeField] = userData.badgeData[badgeField].replace(/%WEEKDAY%/g, weekday.toUpperCase());
            userData.badgeData[badgeField] = userData.badgeData[badgeField].replace(/%WEEKDAY3%/g, `${weekday.toUpperCase()} ${weekday.toUpperCase()} ${weekday.toUpperCase()}`);
        }
